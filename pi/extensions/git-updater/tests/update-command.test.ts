...
describe("UpdateCommand.execute", () => {
  ...
  it("refuses to pull when there are conflicts", async () => {
    ...
    expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Cannot update"), "warning");
  ...
  it("handles pull failure", async () => {
    ...
    expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Update failed"), "error");
