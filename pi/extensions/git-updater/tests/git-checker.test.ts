...
describe("GitChecker.formatStatus", () => {
	...
	it("case A — upstream updates only, clean tree", () => {
		...
		expect(checker.formatStatus(makeStatus({ divergence: { behind: 3, ahead: 0 }, dirty: false }))).toBe(
			"3 updates — /update",
		);
	});
	...
	it("case A — singular update", () => {
		const checker = makeChecker();
		expect(checker.formatStatus(makeStatus({ divergence: { behind: 1, ahead: 0 }, dirty: false }))).toBe(
			"1 update — /update",
		);
	});
